...
// ✅ Semak jika baru je selesai mengundi
$baru_undi     = isset($_SESSION['undi_berjaya']) && $_SESSION['undi_berjaya'] === true;
$jawatan_count = $_SESSION['undi_jawatan_count'] ?? 0;

// Padam session supaya mesej tak muncul semula bila refresh
unset($_SESSION['undi_berjaya']);
unset($_SESSION['undi_jawatan_count']);
?>
...
<div class="hasil-container">
    ...
    <?php if ($baru_undi): ?>
    <!-- ✅ Banner kejayaan — hanya muncul terus selepas mengundi -->
    <div class="banner-berjaya">
        <div class="icon">✅</div>
        <h2>Pengundian Berjaya!</h2>
        <p>Anda telah berjaya mengundi untuk <strong><?php echo $jawatan_count; ?></strong> jawatan. Di bawah adalah pilihan anda.</p>
    </div>
    <?php endif; ?>
    ...
    if ($jenis_pengguna === 'admin') {
        ...
        while ($jawatan = mysqli_fetch_array($jawatan_result)) {
            ...
            if (mysqli_num_rows($hasil_result) > 0) {
                ...
                foreach ($temp_data as $hasil) {
                    ...
                    echo "</div>";
                    if ($percentage > 0) {
                        echo "<div class='chart-bar' style='width: " . $percentage . "%;'>" . round($percentage) . "%</div>";
                    }
                }
            } else {
        ...
        // Pengundi biasa — tunjuk pilihan dia sahaja
        ...
        if (mysqli_num_rows($pilihan_result) > 0) {
            ...
            echo "<p style='font-size: 16px; color: #666;'>Anda belum melakukan pengundian.</p>";
            echo "<a href='undi-calon.php' style='display: inline-block; margin-top: 15px; padding: 10px 25px; background: #7c3aed; color: white; text-decoration: none; border-radius: 8px;'>Pergi Mengundi Sekarang</a>";
            echo "</div>";
        }
    }
    ?>
