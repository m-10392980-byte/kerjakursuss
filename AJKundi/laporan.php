...
<div class="laporan-container">
    ...
    $total_voters_result = mysqli_query($condb, "SELECT COUNT(*) as total FROM pengguna WHERE jenisPengguna='pengundi'");
    $total_voters = mysqli_fetch_array($total_voters_result);

    $total_votes_result = mysqli_query($condb, "SELECT COUNT(*) as total FROM undi");
    $total_votes = mysqli_fetch_array($total_votes_result);

    $total_candidates_result = mysqli_query($condb, "SELECT COUNT(*) as total FROM calon");
    $total_candidates = mysqli_fetch_array($total_candidates_result);

    $unique_voters_result = mysqli_query($condb, "SELECT COUNT(DISTINCT noKPPengundi) as total FROM undi");
    $unique_voters = mysqli_fetch_array($unique_voters_result);

    echo "<div class='stats-grid'>";
    echo "<div class='stat-card'>";
    echo "<div class='stat-label'>Total Pengundi Terdaftar</div>";
    echo "<div class='stat-value'>" . ($total_voters['total'] ?? 0) . "</div>";
    echo "</div>";

    echo "<div class='stat-card'>";
    echo "<div class='stat-label'>Pengundi Aktif</div>";
    echo "<div class='stat-value'>" . ($unique_voters['total'] ?? 0) . "</div>";
    echo "</div>";

    echo "<div class='stat-card'>";
    echo "<div class='stat-label'>Total Vote Diberikan</div>";
    echo "<div class='stat-value'>" . ($total_votes['total'] ?? 0) . "</div>";
    echo "</div>";

    $participation_rate = ($total_voters['total'] ?? 0) > 0 ? round((($unique_voters['total'] ?? 0) / $total_voters['total']) * 100, 1) : 0;
    echo "<div class='stat-card'>";
    ...
    if ($jawatan_result && mysqli_num_rows($jawatan_result) > 0) {
        while ($jawatan = mysqli_fetch_array($jawatan_result)) {
            ...
            $hasil_query = "SELECT calon.idCalon, calon.namaCalon, calon.gambar, COUNT(undi.idCalon) as jumlah_vote
                           FROM calon
            ...
            if ($hasil_result && mysqli_num_rows($hasil_result) > 0) {
                ...
                foreach ($temp_data as $hasil) {
                    ...
                    $gambar = !empty($hasil['gambar']) ? 'gambar/' . htmlspecialchars($hasil['gambar']) : 'gambar/default.png';

                    echo "<div class='candidate-result'>";
                    echo "<div class='candidate-info'>";
                    echo "<img src='" . $gambar . "' alt='" . htmlspecialchars($hasil['namaCalon']) . "' class='candidate-image' onerror=\"this.src='gambar/default.png'\">";
                    echo "<div>";
        ...
        echo "<div class='no-data'>⚠️ Tiada jawatan dijumpai</div>";
    }
